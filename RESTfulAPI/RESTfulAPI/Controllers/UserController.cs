...
namespace RESTfulAPI.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(IRESTfulAPI_Repository repository, IOptions<DatabaseSettings> databaseSettings, IHostingEnvironment hostingEnvironment, IOptions<LinuxSettings> linuxSettings)
        {
        ...
        public async Task<IActionResult> GetUsersAsync(Int32? pageSize = 10, Int32? pageNumber = 1, String name = null)
        {
            var response = new ListModelResponse<UserViewModel>() as IListModelResponse<UserViewModel>;
            ...
            try
            {
                ...
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository
                        .GetUsers(response.PageSize, response.PageNumber, name)
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> GetUserAsync(Int32 id)
        {
            var response = new SingleModelResponse<UserViewModel>() as ISingleModelResponse<UserViewModel>;

            try
            {
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.GetUser(id).ToViewModel();
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> CreateUserAsync([FromBody]UserViewModel value)
        {
            ...
            try
            {
                ...
                {
                    return _RESTfulAPI_Repository.AddUser(value.ToEntity());
                });
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> UpdateUserAsync(Int32 id, [FromBody]UserViewModel value)
        {
            var response = new SingleModelResponse<UserViewModel>() as ISingleModelResponse<UserViewModel>;
            ...
            try
            {
                var entity = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.UpdateUser(id, value.ToEntity());
                });
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = true)]
        public async Task<IActionResult> DeleteUserAsync(Int32 id)
        {
            var response = new SingleModelResponse<UserViewModel>() as ISingleModelResponse<UserViewModel>;
            ...
            try
            {
                var entity = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.DeleteUser(id);
                });
