...
namespace RESTfulAPI.Controllers
{
    ...
    public class PurchaseOrderDetailController : Controller
    {
        ...
        public PurchaseOrderDetailController(IRESTfulAPI_Repository repository, IOptions<DatabaseSettings> databaseSettings, IHostingEnvironment hostingEnvironment, IOptions<LinuxSettings> linuxSettings)
        {
        ...
        public async Task<IActionResult> GetPurchaseOrderDetailsAsync(Int32? pageSize = 10, Int32? pageNumber = 1, String name = null)
        {
            var response = new ListModelResponse<PurchaseOrderDetailViewModel>() as IListModelResponse<PurchaseOrderDetailViewModel>;
            ...
            try
            {
                ...
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository
                        .GetPurchaseOrderDetails(response.PageSize, response.PageNumber, name)
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> GetPurchaseOrderDetailAsync(Int32 id)
        {
            ...
            try
            {
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.GetPurchaseOrderDetail(id).ToViewModel();
                });
            }
            catch (Exception ex)
            {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> DeletePurchaseOrderDetailAsync(Int32 id)
        {
            var response = new SingleModelResponse<PurchaseOrderDetailViewModel>() as ISingleModelResponse<PurchaseOrderDetailViewModel>;
            ...
            try
            {
                var entity = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.DeletePurchaseOrderDetail(id);
                });
