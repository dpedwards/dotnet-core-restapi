...
namespace RESTfulAPI.Controllers
{
    ...
    public class OrderStatusController : Controller
    {
        ...
        public OrderStatusController(IRESTfulAPI_Repository repository, IOptions<DatabaseSettings> databaseSettings, IHostingEnvironment hostingEnvironment, IOptions<LinuxSettings> linuxSettings)
        {
        ...
        public async Task<IActionResult> GetOrderStatuusAsync(Int32? pageSize = 10, Int32? pageNumber = 1, String name = null)
        {
            var response = new ListModelResponse<OrderStatusViewModel>() as IListModelResponse<OrderStatusViewModel>;
            ...
            try
            {
                ...
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository
                        .GetOrderStatuus(response.PageSize, response.PageNumber, name)
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> GetOrderStatusAsync(Int32 id)
        {
            ...
            try
            {
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.GetOrderStatus(id).ToViewModel();
                });
            }
            catch (Exception ex)
            {
