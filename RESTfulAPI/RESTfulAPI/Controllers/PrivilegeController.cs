...
namespace RESTfulAPI.Controllers
{
    ...
    public class PrivilegeController : Controller
    {
        ...
        public PrivilegeController(IRESTfulAPI_Repository repository, IOptions<DatabaseSettings> databaseSettings, IHostingEnvironment hostingEnvironment, IOptions<LinuxSettings> linuxSettings)
        {
        ...
        public async Task<IActionResult> GetPrivilegesAsync(Int32? pageSize = 10, Int32? pageNumber = 1, String name = null)
        {
            var response = new ListModelResponse<PrivilegeViewModel>() as IListModelResponse<PrivilegeViewModel>;
            ...
            try
            {
                ...
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository
                        .GetPrivileges(response.PageSize, response.PageNumber, name)
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> GetPrivilegeAsync(Int32 id)
        {
            ...
            try
            {
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.GetPrivilege(id).ToViewModel();
                });
            }
            catch (Exception ex)
            {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> DeletePrivilegeAsync(Int32 id)
        {
            var response = new SingleModelResponse<PrivilegeViewModel>() as ISingleModelResponse<PrivilegeViewModel>;
            ...
            try
            {
                var entity = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.DeletePrivilege(id);
                });
