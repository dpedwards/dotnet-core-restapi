...
namespace RESTfulAPI.Controllers
{
    ...
    public class CustomerController : Controller
    {
        ...
        public CustomerController(IRESTfulAPI_Repository repository, IOptions<DatabaseSettings> databaseSettings, IHostingEnvironment hostingEnvironment, IOptions<LinuxSettings> linuxSettings)
        {
        ...
        public async Task<IActionResult> GetCustomersAsync(Int32? pageSize = 10, Int32? pageNumber = 1, String name = null)
        {
            var response = new ListModelResponse<CustomerViewModel>() as IListModelResponse<CustomerViewModel>;
            ...
            try
            {
                ...
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository
                        .GetCustomers(response.PageSize, response.PageNumber, name)
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> GetCustomerAsync(Int32 id)
        {
            var response = new SingleModelResponse<CustomerViewModel>() as ISingleModelResponse<CustomerViewModel>;

            try
            {
                response.Model = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.GetCustomer(id).ToViewModel();
                });
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [ApiExplorerSettings(IgnoreApi = false)]
        public async Task<IActionResult> CreateCustomerAsync([FromBody]CustomerViewModel value)
        {
            ...
            try
            {
                ...
                {
                    return _RESTfulAPI_Repository.AddCustomer(value.ToEntity());
                });
            ...
            //
            //
            response.Info = "[Customer] Client " + " " + HttpContext.Connection.RemoteIpAddress.ToString();
        ...
        [ResponseCache(Duration = 30)]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<IActionResult> UpdateCustomerAsync(Int32 id, [FromBody]CustomerViewModel value)
        {
            var response = new SingleModelResponse<CustomerViewModel>() as ISingleModelResponse<CustomerViewModel>;
            ...
            try
            {
                var entity = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.UpdateCustomer(id, value.ToEntity());
                });
        ...
        public async Task<IActionResult> DeleteCustomerAsync(Int32 id)
        {
            var response = new SingleModelResponse<CustomerViewModel>() as ISingleModelResponse<CustomerViewModel>;
            ...
            try
            {
                var entity = await Task.Run(() =>
                {
                    return _RESTfulAPI_Repository.DeleteCustomer(id);
                });
